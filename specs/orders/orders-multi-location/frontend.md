...
[2026-06-14] [lpg-frontend-vue] Follow-on (user-approved): the **assign-driver picker is now scoped to the order's location**. Previously `AssignDialog` listed every open inventory day the caller could see, so for a multi-location operator or admin the driver list wasn't tied to the order's branch. Now:
- `AssignDialog` takes a `storeId` prop (the order's owning branch) and fetches with `inventory.fetchAssignments({ storeId })`; the backend intersects it with the caller's scope (see backend track), so the list = open days at *this order's* location and an API call can't cross branches.
- `inventory/store.ts` `fetchAssignments(extra?: Partial<AssignmentFilter>)` gained an optional narrowing override (default `{}` — `/inventario` unchanged).
- Both call sites pass it: `OrderDetailView` (`order.storeId`) and `OrdersListView` (captures `o.storeId` into `assignTargetStoreId` on open).
Gates green: typecheck ✓ · build ✓.

[2026-06-14] [lpg-frontend-vue] Follow-on (user-approved): **operator location visibility & defaults** across three views, so a store-scoped operator sees/uses their own branch(es) without admin-style guesswork. Shared foundation in the `catalog` module: `service.listStoreAssignments({ all?, userId? })` (added the `userId` filter — backend `?userId=` already supported) + store `myStores` ref and `fetchMyStores(userId)` (the current user's active stores, unique; populated from `auth.user.id`).
- **`/pedidos/nuevo` (OrderCreateView):** the owning-branch selector now shows to operators (was admin/dev only), scoped to **their** stores and **pre-selected to the first**; a single-store operator sees it disabled. `storeValid` now genuinely requires a resolved branch for store-scoped users; an operator assigned to **no** store is blocked with a destructive Alert (instead of submitting a store-less order the server would reject).
- **`/pedidos` (OrdersListView):** operators get a scope line ("Mostrando pedidos de tu(s) tienda(s): …") and the **Tienda column appears for operators with >1 store** (`tableColspan` adjusts); `storeName()` falls back to `myStores` (operators don't fetch all stores).
- **`/inventario` → Disponibilidad (InventoryView):** the store picker is limited to the operator's stores and **defaults to their first** when nothing valid is persisted (an out-of-scope persisted id is rejected). Admin/dev unchanged (all stores).
Note: the `GET /inventory/stores/:id/availability` endpoint is not yet role-scoped on the backend — options are constrained client-side only; a backend scope (like the assignments fix) is a candidate follow-up if needed. Gates green: typecheck ✓ · build ✓. Independent validation: all 3 requirements met (zero-store-operator gap found and fixed).