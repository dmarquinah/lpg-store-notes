...
[2026-06-14] [lpg-backend] Follow-on (user-approved): branch-scoped the **inventory assignment list** that feeds order driver-assignment. `GET /inventory/assignments` was unscoped — any authenticated caller got every store's open days, so an admin (or the assign picker) could see/assign drivers from another branch. Added caller-scoping mirroring `OrdersService.listOrders`:
- New `InventoryService.listAssignmentsForCaller(query, caller)` (the route now calls this; the raw `listAssignments(filter)` stays for trusted internal use — orders' own delivery scoping still calls it). Delivery → pinned to `userId = self` (own days only, overrides any supplied userId); operator/non-global → `storeIds = repo.storeIdsForUser(caller.id)` (**multi-location aware**); admin/developer → global.
- `?storeId=` is treated as a **narrowing** filter intersected with the caller's scope — a tampered id outside the caller's branches yields `[]` (no leak), never a widening grant. Same intersection logic as the orders queue.
- Repo: `AssignmentFilter.storeIds?: number[]` (`inArray`, empty set → no rows) + new `storeIdsForUser`. Route: passes `{id, role}` from `req.user`.
- 8 new scoping tests (`__tests__/scoping.test.ts`): operator single/multi branch, admin all, delivery own-only + can't widen via userId, `?storeId` outside-scope → empty, narrow-within-scope, admin narrow. **Backend 62 → 70 tests.** Gates green: typecheck ✓ · biome lint ✓ · build ✓.